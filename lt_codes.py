...
if __name__ == "__main__":
    ...
    parser.add_argument("-loss", help="percentage of packets lost", default=1.0, type=float)
    ...
    with open(args.filename, "rb") as file:
        ...
        no_of_lost_packets= int(len(file_symbols)*args.loss/100)

        print('Simulating ',args.loss, '% loss:',no_of_lost_packets, 'lost symbols/packets out of ',len(file_symbols) )
        for i in range(no_of_lost_packets):

            random_symbol_index_from_list= random.randint(0,len(file_symbols))
            print('removing index',random_symbol_index_from_list)
            del file_symbols[random_symbol_index_from_list]
